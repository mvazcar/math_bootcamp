...
\begin{itemize}[nosep,leftmargin=1.6em]
  ...
  $C(\{x,z\})=\{z\}$.
  \item If $y\in C(T)$: the same argument against $C(\{x,y\})=\{x\}$.
  \item If $z\in C(T)$: the same argument against $C(\{y,z\})=\{y\}$.
\end{itemize}
Every case fails, so $C(T)=\varnothing$ --- a contradiction. The cycle is
...
\]
So every $\varepsilon$-neighborhood of $x$ contains a strictly better bundle.
\done
...
\slideref{``Lexicographic Preferences Have No Representation'' (MWG 3.C.1)}
...
is uncountable and $\mathbb{Q}$ is countable (Bartle 1.3.6) --- impossible.
\done
...
\slideref{``A Continuous Representation Gives Closed Contours''}
\textbf{Proof.}
...
\]
are then closed for every $x$. By the contour-set characterization
(Kreps 1.14), $\succeq$ is continuous. \done
