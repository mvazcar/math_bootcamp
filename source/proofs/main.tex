...
\textbf{Proof.}
Fix $y\in X$ and let $x_n\in U(y)$ with $x_n\to x\in X$. Each $x_n\in U(y)$
means $u(x_n)\geq u(y)$. Continuity of $u$ gives $u(x_n)\to u(x)$, and limits
preserve weak inequalities, so $u(x)\geq u(y)$; that is, $x\in U(y)$. By the
sequential test, $U(y)$ is closed relative to $X$. Replacing $\geq$ by $\leq$
throughout closes $L(y)$. Closed contour sets are Day 3's definition of
